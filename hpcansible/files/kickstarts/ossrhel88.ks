# Georgios Magklaras - Steelcyber.com Lustre OSS server -- RHEL8.8 version with lustre 2.15.3
...
case "\${#avail_devices[*]}" in
  ...
  1)
     ...
     echo "part pv.02 --fstype=\"lvmpv\" --ondrive=\${os_device} --size=204800" >> /tmp/partition-scheme.cfg
     echo "part pv.01 --fstype=\"lvmpv\" --ondrive=\${os_device} --size 44000 --grow" >> /tmp/partition-scheme.cfg
     echo "volgroup vg00 pv.01"  >> /tmp/partition-scheme.cfg
     echo "logvol swap --fstype=\"swap\" --grow --size=2048 --maxsize=16384 --name=lv_swap --vgname=vg00" >> /tmp/partition-scheme.cfg
...
echo "-------------------------------------------------------------------"
echo "# THIS IS THE STEELCYBER LUSTRE OSS KICKSTART CONFIGURATION SCRIPT#"
echo "-------------------------------------------------------------------"
read -p "Please set the hostname for this OSS server (oss01, oss02): " OSS
...
cobbler-register -s 192.168.14.101 -f $( cat /etc/hostname ) --profile=lustreossrhel88

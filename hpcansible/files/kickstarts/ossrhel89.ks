...
case "\${#avail_devices[*]}" in
  ...
  1)
     ...
     echo "part pv.02 --fstype=\"lvmpv\" --ondrive=\${os_device} --size=189440" >> /tmp/partition-scheme.cfg
     echo "part pv.01 --fstype=\"lvmpv\" --ondrive=\${os_device} --size 24576 --grow" >> /tmp/partition-scheme.cfg
     echo "volgroup vg00 pv.01"  >> /tmp/partition-scheme.cfg
     echo "logvol swap --fstype=\"swap\" --grow --size=2048 --maxsize=12288 --name=lv_swap --vgname=vg00" >> /tmp/partition-scheme.cfg
     echo "logvol /  --fstype="ext4" --grow --maxsize=8192 --size=8192 --name=lv_root --vgname=vg00" >> /tmp/partition-scheme.cfg
     echo "logvol /var/log  --fstype="ext4" --grow --maxsize=4096 --size=4096 --name=lv_var_log --vgname=vg00" >> /tmp/partition-scheme.cfg
     ;;
  2)
...
cobbler-register -s 192.168.14.101 -f $( cat /etc/hostname ) --profile=ossrhel89

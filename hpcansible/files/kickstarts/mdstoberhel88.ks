# Georgios Magklaras - Steelcyber.com Lustre MDS server -- RHEL8.8 version with lustre 2.15.3
...
case "\${#avail_devices[*]}" in
  ...
  1)
     ...
     echo "part /boot --fstype=\"ext4\" --asprimary --ondrive=\${os_device} --size=512" >> /tmp/partition-scheme.cfg
     echo "part /lustre/mgs --fstype=\"ext4\" --asprimary --ondrive=\${os_device} --size=2048" >> /tmp/partition-scheme.cfg
     echo "part /lustre/mdt1 --fstype=\"ext4\" --asprimary --ondrive=\${os_device} --size=133120" >> /tmp/partition-scheme.cfg
     echo "part pv.01 --fstype=\"lvmpv\" --ondrive=\${os_device} --size 50000 --grow" >> /tmp/partition-scheme.cfg
     echo "volgroup vg00 pv.01"  >> /tmp/partition-scheme.cfg
     echo "logvol swap --fstype=\"swap\" --grow --size=2048 --maxsize=16000 --name=lv_swap --vgname=vg00" >> /tmp/partition-scheme.cfg
     echo "logvol /  --fstype="ext4" --grow --maxsize=25000 --size=10240 --name=lv_root --vgname=vg00" >> /tmp/partition-scheme.cfg
     echo "logvol /var/log  --fstype="ext4" --grow --maxsize=10240 --size=4096 --name=lv_var_log --vgname=vg00" >> /tmp/partition-scheme.cfg
     ;;
  2)
...
echo "-------------------------------------------------------------------"
echo "# THIS IS THE STEELCYBER LUSTRE MDS KICKSTART CONFIGURATION SCRIPT#"
echo "-------------------------------------------------------------------"
read -p "Please set the hostname for this MDS server (mds01, mds02): " MDS

echo "network --bootproto=dhcp --noipv6 --hostname $MDS --activate" > /tmp/ppihostname.txt
...
#Install the e2fsprogs and ldiskfs tools ONLY from https://downloads.whamcloud.com/public/e2fsprogs/latest/el8/RPMS/x86_64/ (or the local mirror), NOT from the distro
dnf -y remove e2fsprogs*
dnf -y install http://192.168.122.33/lustre/2.15.3/el8.8/server/e2fsprogs-1.47.0-wc4.el8.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/e2fsprogs-devel-1.47.0-wc4.el8.x86_64.rpm  http://192.168.122.33/lustre/2.15.3/el8.8/server/e2fsprogs-libs-1.47.0-wc4.el8.x86_64.rpm  http://192.168.122.33/lustre/2.15.3/el8.8/server/libcom_err-1.47.0-wc4.el8.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/libcom_err-devel-1.47.0-wc4.el8.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/libss-1.47.0-wc4.el8.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/libss-devel-1.47.0-wc4.el8.x86_64.rpm


#Install the patched Lustre kernel
dnf -y install http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-core-4.18.0-477.10.1.el8_lustre.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-4.18.0-477.10.1.el8_lustre.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-devel-4.18.0-477.10.1.el8_lustre.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-headers-4.18.0-477.10.1.el8_lustre.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-modules-4.18.0-477.10.1.el8_lustre.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-modules-extra-4.18.0-477.10.1.el8_lustre.x86_64.rpm http://192.168.122.33/lustre/2.15.3/el8.8/server/kernel-modules-internal-4.18.0-477.10.1.el8_lustre.x86_64.rpm


#Make the Lustre patched kernel the default one (This will be the second kernel after the default
#RHEL 8.8 one 
grub2-set-default 1

#Install the DRBD kernel module
dnf -y install vim 
#dnf -y install http://192.168.14.101/drbd/kmod-drbd90-9.1.11-1.el8_6.elrepo.x86_64.rpm
#dnf -y install drbd-utils

#Now we will install Lustre server MDS packages version 2.15.1 for RHEL 8.6
#dnf -y localinstall http://192.168.14.101/lustre/2.15.1/el8.6/server/kmod-lustre-2.15.1-1.el8.x86_64.rpm


#Finally register the new node with cobbler based on the hostname we have already set
#cobbler-register -s 192.168.14.101 -f $( cat /etc/hostname ) --profile=rhel86-r7525-gpu-ppi-compute-b
